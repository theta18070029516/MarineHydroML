...
2. **外边界硬约束，内边界由场诱导**：PI-sampler 强制 $P=0$ 于 $r=5$，训练样本的内边界通量从构造场自动求导得到。
...
有效批量为 $B=\texttt{num\_repeats}\times\texttt{sample\_size}$。当前默认配置下，$N_\theta=128$、$N_r=32$、$N_{\mathrm{probe}}=1024$。
...
$$
\mathcal L_{\mathrm{OL}}=\operatorname{MSE}(\hat z_P,z_P).
$$
