...
def load_fe_state(
    config: AnnulusConfig,
    key: Array,
    param_name: str = "fe_params.msgpack",
) -> TrainState:
    fe_state, _ = create_fe_state(config, key)
    param_path = config.output_dir / param_name

    if not param_path.exists():
        raise FileNotFoundError(f"Cannot find FE params: {param_path}")

    fe_bytes = param_path.read_bytes()
    fe_params = serialization.from_bytes(fe_state.params, fe_bytes)
    return fe_state.replace(params=fe_params)
...
def compute_trunk_laplacian_basis(
    params: dict,
    state: TrainState,
    coords: Array,
) -> tuple[Array, Array]:
    """
    Compute trunk basis and Laplacian of trunk basis.
...
def fe_loss_fn(
    ...
    # ------------------------------------------------------------
    # 1. Encode u and f from pod grid
    # ------------------------------------------------------------
    latent_u = state.apply_fn(
        {"params": params},
        u_in_norm,
    ...
    # ------------------------------------------------------------
    # 2. Data reconstruction loss on probe_coords
    # ------------------------------------------------------------
    ...
    data_loss = loss_u + loss_f

    # ------------------------------------------------------------
    # 3. Physics loss on the SAME probe_coords
    #
    # PDE:
    #     Δu_pred - k^2 u_pred - f = 0
    #
    # Important:
    #     pred_u_norm is normalized.
    #     u_phys = std_u * pred_u_norm + mean_u
    #     lap_u_phys = std_u * lap_u_norm
    # ------------------------------------------------------------
    basis_probe, lap_basis_probe = compute_trunk_laplacian_basis(
        params=params,
        state=state,
        coords=coords,
    )

    u_probe_norm_phys = jnp.einsum(
        "bd,nd->bn",
        latent_u,
        basis_probe,
    ) / jnp.sqrt(n_basis)

    lap_u_probe_norm = jnp.einsum(
        "bd,nd->bn",
        latent_u,
        lap_basis_probe,
    ) / jnp.sqrt(n_basis)

    u_probe_phys = denormalize_u(u_probe_norm_phys, normalizer)
    lap_u_probe_phys = normalizer.std_u * lap_u_probe_norm

    residual = (
        lap_u_probe_phys
        - (k_values[:, None] ** 2) * u_probe_phys
        - f_out_phys
    )

    # residual has the same physical dimension as f.
    # Divide by std_f to make it comparable to normalized-space f loss.
    residual_norm = residual / normalizer.std_f

    phys_loss = jnp.mean(residual_norm ** 2)
...
def fe_physics_eval_step(
    ...
    u_probe_phys = denormalize_u(u_probe_norm, normalizer)
    lap_u_probe_phys = normalizer.std_u * lap_u_probe_norm
    ...
    return jnp.mean(residual_norm ** 2)
...
def ol_eval_physical_step(
    ...
    return rl2_error(u_pred, u_ref).mean()
...
def train_fe(config: AnnulusConfig) -> tuple[TrainState, FieldNormalizer]:
    ...
    for step in range(config.fe_steps):
        ...
        state, loss = fe_train_step(
            ...
            u_out_norm,
            f_out_norm,
            batch.u_probe,          # physical u on probe_coords
            batch.f_probe,          # physical f on probe_coords
            batch.probe_coords,     # same coords for data loss and physics loss
            batch.k_values,
            normalizer,
            config.n_basis,
            config.fe_phys_weight,
        )
        ...
        if step % 500 == 0:
            ...
            phys_eval = fe_physics_eval_step(
                state,
                u_in_norm,
                batch.probe_coords,
                batch.f_probe,
            ...
            print(
                ...
                f"rl2_u={float(ru):.4e} "
                f"rl2_f={float(rf):.4e} "
                f"phys={float(phys_eval):.4e}"
            )
    
        if step % 10000 == 0 and step > 0:
            save_params(state.params, config.output_dir / "fe_params_phys.msgpack")
...
def train_ol(
    ...
    # 2. Load pretrained FE
    if fe_state is None:
        fe_state = load_fe_state(
            config,
            key_fe_init,
            param_name="fe_params_physv2.msgpack",
        )

    # 3. Create Transformer state
    ol_state, _ = create_ol_state(config, key_ol_init)
    ...
    for step in range(config.ol_steps+1):
        ...
        # ------------------------------------------------------------
        # Important:
        # FE was trained in normalized field space.
        # Therefore, branch inputs must be normalized.
        # ------------------------------------------------------------
        ...
        # ------------------------------------------------------------
        # Extract FE latents.
        # FE params are fixed; Transformer is the only trainable module.
        # ------------------------------------------------------------
        ...
        # ------------------------------------------------------------
        # Tokenize inputs for Transformer.
        # ------------------------------------------------------------
        f_tokens = make_source_tokens(latent_f, config)
        cond_tokens = make_condition_tokens(batch, config)
        ...
        if step % 500 == 0:
            ...
            phys_rl2 = ol_eval_physical_step(
                ol_state,
                fe_state,
                f_tokens,
                cond_tokens,
            ...
            print(
                f"[OL] step={step:07d} "
                f"loss={float(loss):.4e} "
                f"latent_rl2={float(latent_rl2):.4e} "
                f"u_phys_rl2={float(phys_rl2):.4e}"
            )

        if step % 10000 == 0 and step > 0:
            save_params(
                ol_state.params,
                config.output_dir / "ol_params_physv2.msgpack",
            )
