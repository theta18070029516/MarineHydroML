...
class PolarBNNParams(NamedTuple):
    """Parameters of the periodic base field U(r, theta).
...
def sample_bnn_params(
    ...
    """Sample the single-hidden-layer periodic Fourier BNN for U."""
...
def evaluate_polar_prior(
    ...
    r"""Evaluate the pressure prior and all analytic PDE derivatives.

    The base BNN is

        U = sqrt(2/H) sum_j a_j cos(phi_j),

    and the pressure prior is ``P = c_sigma_r (r-r_outer) U``.  The returned
    ``q`` and ``q_r`` are physical derivatives ``P_r`` and ``P_rr``.  The only
    difference between the two ablation variants is the deterministic scalar
    ``c_sigma_r`` supplied by :meth:`PolarAnnulusConfig.pressure_scale`.
    """
    ...
    u = scale * jnp.sum(weighted_out * cos_phase, axis=-1)
    u_r = scale * jnp.sum(
        weighted_out * (-sin_phase) * w_r[:, None, :] * config.drhat_dr,
        axis=-1,
    )
    u_rr = scale * jnp.sum(
        weighted_out
        * (-cos_phase)
        * (w_r[:, None, :] * config.drhat_dr) ** 2,
        axis=-1,
    )
    u_theta2 = scale * jnp.sum(
        weighted_out
        * (
            -cos_phase * phase_theta**2
            - sin_phase * phase_theta2
        ),
        axis=-1,
    )

    prior_scale = jnp.asarray(config.pressure_scale(sigma_r), dtype=u.dtype)
    outer_mask_physical = radius[None, :] - config.r_outer
    p = prior_scale * outer_mask_physical * u
    q = prior_scale * (u + outer_mask_physical * u_r)
    q_r = prior_scale * (2.0 * u_r + outer_mask_physical * u_rr)
    p_theta2 = prior_scale * outer_mask_physical * u_theta2
...
def evaluate_direct_q_prior_flux(
    params: PolarBNNParams,
    coords_hat: Array,
) -> Array:
    """Return ``g_n=-U`` for the reference direct-q prior using the same BNN.

    This diagnostic function is not used to generate training data.  It exists
    only to verify the pre-registered boundary-RMS calibration against the
    current q-prior under identical sampled parameters and boundary points.
    """
    if coords_hat.ndim != 2 or coords_hat.shape[-1] != 2:
        raise ValueError("coords_hat must have shape [N, 2].")
    ...
    r_hat = coords_hat[:, 1]
    phase = (
        params.w1[:, 0, :][:, None, :] * jnp.sin(theta)[None, :, None]
        + params.w1[:, 1, :][:, None, :] * jnp.cos(theta)[None, :, None]
        + params.w1[:, 2, :][:, None, :] * r_hat[None, :, None]
        + params.b1[:, None, :]
        - jnp.pi / 4.0
    )
    scale = jnp.sqrt(2.0 / params.w1.shape[-1])
    u = scale * jnp.sum(params.w2[:, None, :] * jnp.cos(phase), axis=-1)
    return -u
...
def sample_batch(key: Array, config: PolarAnnulusConfig) -> SampleBatch:
    ...
    for group_key, (sigma_theta, sigma_r) in zip(group_keys, group_specs):
        ...
        pod_eval = evaluate_polar_prior(
            params, pod_coords, k_values, sigma_r, config
        )
        probe_eval = evaluate_polar_prior(
            params, probe_coords, k_values, sigma_r, config
        )
        bnd_eval = evaluate_polar_prior(
            params, bnd_coords_single, k_values, sigma_r, config
        )
...
def build_field_normalizer_online(
    ...
    # The registered experiment is float32. Explicit float32 avoids misleading
    # x64-truncation warnings on the default JAX configuration while preserving
    # the numerical behavior of the polar_v2 run.
    p_sum_sq = jnp.asarray(0.0, dtype=jnp.float32)

    f_sum = jnp.asarray(0.0, dtype=jnp.float32)
    f_sum_sq = jnp.asarray(0.0, dtype=jnp.float32)
    ...
    for _ in range(config.normalizer_batches):
        ...
        p_values = p_values.astype(jnp.float32)
        f_values = f_values.astype(jnp.float32)
