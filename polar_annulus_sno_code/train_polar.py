...
def save_operator_history(
    ...
    csv_path = out_dir / "operator_training_history.csv"
    csv_temp_path = out_dir / "operator_training_history.tmp.csv"
    with csv_temp_path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=OPERATOR_HISTORY_COLUMNS)
        writer.writeheader()
        writer.writerows(history)
    csv_temp_path.replace(csv_path)

    npz_path = out_dir / "operator_training_history.npz"
    npz_temp_path = out_dir / "operator_training_history.tmp.npz"
    arrays = {
        name: np.asarray([record[name] for record in history])
        for name in OPERATOR_HISTORY_COLUMNS
    }
