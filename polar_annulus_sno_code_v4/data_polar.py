...
class SampleBatch(NamedTuple):
    ...
    k_values: Array         # [B]
    sigma_theta: Array      # Per-sample angular prior scale, [B]
    sigma_r: Array          # Per-sample radial prior scale, [B]
...
def sample_sigma_pairs(
    ...
    """Independently draw one (sigma_theta, sigma_r) pair per sample."""
    if batch_size <= 0:
        raise ValueError("batch_size must be positive.")
    key_theta, key_r = jax.random.split(key)
    sigma_theta = _uniform_or_constant(
        key_theta,
        batch_size,
        config.sigma_theta_range,
    )
    sigma_r = _uniform_or_constant(
        key_r,
        batch_size,
...
def sample_bnn_params(
    key: Array,
    batch_size: int,
    sigma_theta: Array | float,
    sigma_r: Array | float,
    config: PolarAnnulusConfig,
) -> PolarBNNParams:
    """Sample Fourier-BNN parameters with per-sample prior scales."""
    key_w1, key_b1, key_w2 = jax.random.split(key, 3)
    sigma_theta = jnp.asarray(sigma_theta, dtype=jnp.float32)
    sigma_r = jnp.asarray(sigma_r, dtype=jnp.float32)
    if sigma_theta.ndim == 0:
        sigma_theta = jnp.broadcast_to(sigma_theta, (batch_size,))
    if sigma_r.ndim == 0:
        sigma_r = jnp.broadcast_to(sigma_r, (batch_size,))
    if sigma_theta.shape != (batch_size,) or sigma_r.shape != (batch_size,):
        raise ValueError(
            "sigma_theta and sigma_r must be scalars or arrays with shape "
            f"({batch_size},)."
        )
    feature_sigmas = jnp.stack(
...
def sample_batch(key: Array, config: PolarAnnulusConfig) -> SampleBatch:
    ...
    Every sample independently draws sigma_theta and sigma_r from their
    configured uniform ranges. The same BNN parameters are evaluated on the POD,
    probe, and inner-boundary coordinates, guaranteeing exact sample consistency.
    Generation is chunked only to bound peak memory; it does not alter the law.
    """
    key_probe, key_sigma, key_params, key_k = jax.random.split(key, 4)
    ...
    if config.k_max == config.k_min:
        all_k_values = jnp.full(
            (config.sample_size,),
            config.k_min,
            dtype=jnp.float32,
        )
    else:
        all_k_values = jax.random.uniform(
            key_k,
            (config.sample_size,),
            minval=config.k_min,
            maxval=config.k_max,
    ...
    chunk_size = min(config.sample_size, config.prior_generation_chunk_size)
    chunk_starts = list(range(0, config.sample_size, chunk_size))
    parameter_keys = jax.random.split(key_params, len(chunk_starts))
    ...
    for parameter_key, start in zip(parameter_keys, chunk_starts):
        stop = min(start + chunk_size, config.sample_size)
        chunk_k_values = all_k_values[start:stop]
        params = sample_bnn_params(
            parameter_key,
            stop - start,
            sigma_theta[start:stop],
            sigma_r[start:stop],
            config,
        )

        pod_eval = evaluate_polar_prior(
            params, pod_coords, chunk_k_values, config
        )
        probe_eval = evaluate_polar_prior(
            params, probe_coords, chunk_k_values, config
        )
        bnd_eval = evaluate_polar_prior(
            params, bnd_coords_single, chunk_k_values, config
        )
...
def build_field_normalizer_online(
    ...
    # Accumulate in the configured JAX float32 runtime. Requesting float64 when
    # x64 is disabled only emits warnings and is silently truncated anyway.
    ...
    mean_f_value = f_sum / float(f_count)

    variance_f = (
        f_sum_sq / float(f_count)
        - jnp.square(mean_f_value)
    )

    variance_f = jnp.maximum(
        variance_f,
        0.0,
    )

    mean_f = mean_f_value.astype(jnp.float32)
    std_f = jnp.sqrt(
        variance_f
    ).astype(jnp.float32)
